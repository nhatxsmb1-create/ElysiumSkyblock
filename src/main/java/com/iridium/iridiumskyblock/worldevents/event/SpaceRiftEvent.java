...
public class SpaceRiftEvent extends IslandWorldEvent {
    ...
    private static final double BASE_HP_PER_MOB = 60.0;
    ...
    @Override public void start(Runnable onFinish) {
        broadcast("§5🌀 A §5Space Rift §fhas appeared above your island!");
        countdown("§5Rift tearing open...", () -> openRift(onFinish));
    }
    ...
    private void openRift(Runnable onFinish) {
        ...
        BukkitRunnable swirl=new BukkitRunnable(){
            double a=0; int e=0; @Override public void run(){
                e+=2; if(e>WARN){cancel();return;}
                a+=18;
                for(int i=0;i<3;i++){double ang=Math.toRadians(a+i*120);
                    Location p=rift.clone().add(Math.cos(ang)*2,0,Math.sin(ang)*2);
                    fx(p,Particle.PORTAL,5,0.1,0.3,0.1,0.3); fx(p,Particle.DRAGON_BREATH,2,0.1,0.1,0.1,0.02);}
            }
        };
    ...
    private void runWaves(World world, Location rift, Runnable onFinish) {
        ...
        int[] wavesDone={0};
        double mobHP=scaledHP(BASE_HP_PER_MOB);
        ...
        Runnable spawnWave=()->{
            ...
            for(int i=0;i<MOB_PER_WAVE;i++){
                ...
                e.setCustomName("§5Void Entity §7[W"+wavesDone[0]+"]"); e.setCustomNameVisible(true);
        ...
        new BukkitRunnable(){
            int e=0; @Override public void run(){
                ...
                if(mobs.stream().noneMatch(Entity::isValid)&&wavesDone[0]>=WAVES){
                    ...
                    if(hasLootBonus()){world.dropItemNaturally(drop,named(Material.ELYTRA,"§5§lVoid Wings"));
                        broadcast("§d🌀 §lBonus loot! §r§dVoid Wings dropped!");}
                ...
                if(e>=timeout){cancel();mobs.forEach(m->{if(m.isValid())m.remove();});
                    broadcast("§c🌀 The Rift sealed itself..."); logResult("TIMEOUT"); onFinish.run();}
            }
