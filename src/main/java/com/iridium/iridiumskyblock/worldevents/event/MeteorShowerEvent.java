...
public class MeteorShowerEvent extends IslandWorldEvent {
    ...
    @Override public void start(Runnable onFinish) {
        broadcast("§e🌠 A §eMeteor Shower §fis incoming! Watch the skies!");
        countdown("§eMeteors inbound...", ()->shower(onFinish));
    }

    private void shower(Runnable onFinish) {
        World world=center.getWorld(); Random rng=new Random();
        new BukkitRunnable(){
            int n=0; @Override public void run(){
                if(n>=COUNT){cancel();broadcast("§e🌠 The shower has passed.");logResult("COMPLETE");onFinish.run();return;}
                n++;
    ...
    private void dropMeteor(World world, Location ground, Random rng) {
        broadcast("§e🌠 A meteor incoming!");
        sound(ground.clone().add(0,50,0),Sound.ENTITY_FIREWORK_ROCKET_LAUNCH,1f,0.4f);
        new BukkitRunnable(){
            double y=50; @Override public void run(){
                y-=3; Location cur=ground.clone().add(0,y,0);
                fx(cur,Particle.FLAME,3,0.3,0.1,0.3,0.05); fx(cur,Particle.LAVA,2,0.2,0.1,0.2,0.01);
    ...
    private void impact(World world, Location loc, Random rng) {
        ...
        fx(loc,Particle.EXPLOSION_EMITTER,2,0.5,0.5,0.5,0.0);
        fx(loc,Particle.LAVA,10,0.5,0.5,0.5,0.1);
        broadcast("§6☄ Meteor at §e("+loc.getBlockX()+", "+loc.getBlockZ()+")§6! Grab in §c"+PICKUP_S+"s§6!");

        // Loot scales with instability
        Material[] base={Material.DIAMOND,Material.EMERALD,Material.GOLD_INGOT,Material.IRON_INGOT};
        Material mat=base[rng.nextInt(base.length)];
        int qty=hasLootBonus()?1+rng.nextInt(3)+2:1+rng.nextInt(3);
        org.bukkit.inventory.ItemStack loot=named(mat,"§6§lMeteor "+mat.name(),qty);
        Item lootItem=world.dropItem(loc.clone().add(0,1,0),loot); lootItem.setPickupDelay(0);
